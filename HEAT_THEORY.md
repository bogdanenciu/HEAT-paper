...
$$
E_{\mathrm{HEAT}}(z)^2 = \Omega_b F_0 (1+z)^3 + \Omega_\Lambda,
...
In deep-MOND equilibrium $R \propto (G M_b / a_0)^{1/2}$. Combined with
$a_0(z) = c H(z) / (2\pi)$ at fixed baryonic mass,

$$
\frac{R(z)}{R_0} = \left[\frac{H(z)}{H_0}\right]^{-1/2} = (1+z)^{-3/4} \left[\frac{\Omega_m(z)}{\Omega_m(0)}\right]^{1/4},
$$

with matter-era exponent $-3/4$ (matching van der Wel et al. 2014) and
normalisation asymptote
...
| `sparc-pub` | `publication/sparc_publication_quality.py` | Fig. 4 (chi-squared histograms) |
